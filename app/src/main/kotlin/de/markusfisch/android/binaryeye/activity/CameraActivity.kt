...
import de.markusfisch.android.binaryeye.validing.Validing
...
class CameraActivity : AppCompatActivity() {
	...
	lateinit var inputIpPort: TextInputEditText

	val DEFAULT_IP_PORT: String = "192.168.1.5:8080"

	// разрешения на использзованиме камеры
	...
	override fun onCreate(state: Bundle?) {
		...
		inputIpPort = findViewById(R.id.inputIpPort) as TextInputEditText

		inputIpPort.setText(DEFAULT_IP_PORT)
				...
				camera.setPreviewCallback { frameData, _ ->
					if (decoding) {
						...
						ZxingCpp.readByteArray(
							...
							if (validing.isValidIpAddressPort(inputIpPort.text.toString())) {
								try {
									sendUDPRequest(
										inputIpPort.text.toString().substringBefore(":"),
										inputIpPort.text.toString().substringAfter(":").toInt(),
										result.text
									)
									Log.d("frameDataMy", result.text)
								} catch (_: Exception) {}
							}
						}
					}
	...
	// отправка в порт
	fun sendUDPRequest(host: String, port: Int, message: String, responseFlag: Boolean = false) {
		Thread {
